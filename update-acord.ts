...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('export function generateAcord125FullForm(data: ApplicationPacketData): string {') ||
      lines[i].includes('export function generateAcord125Page1(data: ApplicationPacketData): string {')) {
    startIdx = i;
  }
  if (lines[i].includes('export async function generateApplicationPacketHTML')) {
    endIdx = i;
    break;
  }
}
...
if (startIdx !== -1 && endIdx !== -1) {
  const newFunc = `export function generateAcord125Page1(data: ApplicationPacketData): string {
...
`;
  ...
  // Also we need to modify the generateApplicationPacketHTML function to use generateAcord125Page1
  const modifiedLines = [
    ...lines.slice(0, startIdx),
    newFunc,
    ...lines.slice(endIdx).map(line => line.replace('generateAcord125FullForm(dataWithQR)', 'generateAcord125Page1(dataWithQR)'))
  ];

  fs.writeFileSync('src/lib/services/pdf/ApplicationPacketPDF.ts', modifiedLines.join('\n'));
  console.log("Updated function successfully");
} else {
  console.error("Could not find start/end marks", startIdx, endIdx);
}
